...
7. **Optimization** (feedback from first hands-on review, 2026-07-02):
   ...
   - **External checks: map doesn't load** — the local zip server works but MapCheck/bs-parity don't pick the file up. Investigate: `?url=` parameter handling of both tools, CORS/mixed-content in the embedded WebView vs. real browser, MapCheck's actual URL-loading API. Fix delivery or fall back to guided manual load.
